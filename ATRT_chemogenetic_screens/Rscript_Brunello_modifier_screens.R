...
#get data
getwd()
setwd("\\\\fileserv1/groups/Neuro-Oncology/Clinical Neuro-Oncology/Daniel/B22-ATRT screens/Modifier_screens/Brunello/MAGeCK Flute/final_analysis/data")

#make scatter volcano for BT16 and CHLA06 to validate knockout-screens DMSO vs plasmid
essential.genes = read.delim("CEGv2.txt", header = T)
essential.genes = essential.genes$GENE
nonessential.genes = read.delim("NEGv1.txt", header = T)
nonessential.genes = nonessential.genes$GENE
all.screens.vs.plasmid = read.delim("All_Screens_vs_plasmid.gene_summary.txt")
data.BT16.DMSOvsplasmid.volcano = all.screens.vs.plasmid[,1:8]
data.CHLA06.DMSOvsplasmid.volcano = all.screens.vs.plasmid[,-(3:20)]
data.CHLA06.DMSOvsplasmid.volcano = data.CHLA06.DMSOvsplasmid.volcano[,1:8]
data.BT16.DMSOvsplasmid.volcano %>%
  ggplot(aes(x=BT16_DMSO.beta, y= -log10(BT16_DMSO.wald.p.value), label = Gene)) +
  geom_point()+
  geom_point(data = data.BT16.DMSOvsplasmid.volcano %>% filter(Gene %in% essential.genes), color = "red") +
  ylim(0,8)+
  theme_classic()
data.CHLA06.DMSOvsplasmid.volcano %>%
  ggplot(aes(x=CHLA06_DMSO.beta, y= -log10(CHLA06_DMSO.wald.p.value), label = Gene)) +
  geom_point()+
  geom_point(data = data.CHLA06.DMSOvsplasmid.volcano %>% filter(Gene %in% essential.genes), color = "red") +
  ylim(0,8)+
  theme_classic()

###
data.BT16.drugcommonvsplasmid = ReadBeta("BT16_drug_common_vs_plasmid.gene_summary.txt")
data.BT16.DMSOvsplasmid = data.BT16.DMSOvsplasmid.volcano[,c(1,3)]
data.BT16.scatter = cbind(data.BT16.drugcommonvsplasmid, data.BT16.DMSOvsplasmid$BT16_DMSO.beta)
colnames(data.BT16.scatter)[colnames(data.BT16.scatter) == 'BT16_drug_common'] <- 'CDKi'
...
Scatter.BT16.CDKi =ScatterViewCustom(data.BT16.scatter, x= "DMSO", y= "CDKi", 
                                     ...
                                     toplabels = c("FBXW7","RB1","AMBRA1", "TINF2", "FZR1", "UBA1", "CPED1", "AKIRIN2", "DNAJC9", "RNF39"),
...
Scatter.CHLA06.CDKi =ScatterViewCustom(data.CHLA06.scatter, x= "DMSO", y= "CDKi", 
                                     label = "Gene",size =2,groups = c("top", "bottom"), top=0,
                                     toplabels = c("FBXW7","RB1","AMBRA1", "TINF2", "FZR1", "UBA1", "CPED1", "RNF39"),
                                     auto_cut_diag  = 2, group_col = c("#e41a1c", "#377eb8"),
                                     alpha = 0.5, display_cut = TRUE)
